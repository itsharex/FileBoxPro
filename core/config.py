...
class Settings:
    # 项目版本
    VERSION = config('VERSION', default='1.7')
    # Debug模式
    DEBUG = config('DEBUG', default=False)
    # 项目根目录
    BASE_DIR: str = Path(__file__).resolve().parent.parent
    # 端口
    PORT = config('PORT', default=12345)
    # 端口
    WORKERS = config('WORKERS', default=1)
    # 静态文件路径
    STATIC_DIR = config('STATIC_DIR', default='assets')
    # APP路径
    APPS_DIR = config('APPS_DIR', default='apps')
    # 数据库信息
    DATABASE_URL = config('DATABASE_URL', default=f"sqlite+aiosqlite:///filebox.db")
    # 开启上传
